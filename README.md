...
为了验证系统有效性，我需要一个深度的测试样板。基于基金 2026 Q2 的 13F 披露，**NBIS（占比 51%）** 处于 AI 基建价值链（L2 云与平台）的枢纽位置，是衔接上游算力与下游应用的核心观测点。

**实证数据基础 (as-of 2026-09)**：
在进行链路推演前，系统首先接入并锁定了以下时间点的数据事实：

| 维度 | 数据内容 | 数据来源与可信度 | 时间点 |
| :--- | :--- | :--- | :--- |
| **供给端 (D1)** | 海关 HS8517 光模块出口数据（出货量显著上升） | 海关总署月报（[P1 级 ↗](docs/21-数据来源与处理方法说明.md#海关数据)） | 2026年 5-6月 |
| **需求端 (D3)** | 前沿 Lab (如 Reflection) 大额融资到位并转化为算力订单 | 新闻公告 / Form D（[P1 级 ↗](docs/21-数据来源与处理方法说明.md#融资数据)） | 2025 Q4 - 2026 Q2 |
| **公司指引** | NBIS 管理层给出的上电量预期与指引 | Q1 财报电话会转录 | 2026 Q1 及此前 |
...
1. **PROPOSE（确定点边与候选）**：
   ...
   *   **边与权重**：这两端通过 6 条独立通道相连（[`edge_registry` ↗](pipeline/SCHEMA.md#tbl-edge_registry)）。为了量化影响，系统引入了节点质量的五因子（r 可靠度 / e 独家性 / l 领先性 / s 信噪比 / φ 频率），并设计了双重打分公式：
       *   **可交易度 (Tradable)** = `c · r · s · φ`。例如，海关数据（P1级，高频低噪）计算得出高可交易度，直接支撑多头仓位。
