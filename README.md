...
1. **PROPOSE（确定点边与候选）**：
   ...
   *   **最小割（哪一环断了仓位最痛）**：左边 6 条证据都能支撑「持有 NBIS」。少一条（比如光模块数据暂时没有），另外几条还在，判断不必立刻翻。但供给那几条（光模块、GPU、capex）都要先经过同一件事：**机房有没有真正上电**。上电没发生，这几条供给证据同时作废，只剩下公司自己的 backlog 和下游 lab 有没有钱买算力。所以上电是最该盯的那一环（图上红框）。
