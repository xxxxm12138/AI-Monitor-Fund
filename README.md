...
> **核心思路**：事实入账 → 展开互斥读法 → 财报前选一条 → 披露后对错都写回。资产是可回放的判断账，不是单次猜中。

基于 2026 Q2 的 13F，**NBIS 占 51%**。决策窗口：**2026-05-13**（Q1 股东信）→ **2026-08-12**（Q2 披露），间隔 91 天。海关 HS8517 未当实测。

**路径：** 事实（供给热 / 有单 / 并网与 Q1 冷）→ 候选 **A 天花板 · B 时差 · C 打穿全年** → 选 **B** → Q2：**B 中环比，C 不中全年**。

<div align="center">
<img src="assets/readme/nbis.svg" alt="NBIS 供需对账" width="85%">
</div>

### 4.1 REPRESENT · 事实入账（不做读法）

季报走 **Code**，股东信 / 电话会走 **LLM**，写入 [`fct_quant`](pipeline/SCHEMA.md#tbl-fct_quant)。回测只准用 `knowledge_time` 当时已可知的行。

| 链 | 事实（Q2 披露前） | 可知日 |
| :--- | :--- | :--- |
| **供给** | 旭创 Q1 **195 亿 +192%**（`r320`）；新易盛 Q1 **83 亿 +106%**（`r328`） | 04-17 / 04-24 |
| **需求** | Reflection 融资 **$2B** → 与 NBIS 合同 **$1B**（`r404` / `r406`）；OpenAI $122B、Anthropic $65B | 至 07-14 |
| **公司** | 合同电力 **>3.5 GW**、年底指引 **>4 GW**；并网 **800 MW–1 GW**（灯已亮 ≠ 合同额度）；Q1 AI cloud **$389.7M**；FY 指引 **$3.0–3.4B** | 05-13 |

抽取只对字段负责：旭创一季报 → `r320` actual；Q1 股东信 “more than 4 GW” → `r255` guidance。海关月报仍是 placeholder，不进后面的分叉。

### 4.2 PROPOSE · 同一组事实，三条互斥路

三条**平行链**（不是漏斗）：供给先汇「集群在建」；需求走 lab 采购；并网是公司自己的产能边，不接光模块。[`graph_data.py`](pipeline/graph_data.py)

<div align="center">
<img src="assets/readme/evidence-nbis.svg" alt="供给汇集群在建；并网单独" width="88%">
</div>

冲突就一句：外面热，公司并网和上一季收入看起来冷。财报前对「冷」有三种读法：

| 候选 | 读法 | 预告 Q2 | 财报前动作 |
| :--- | :--- | :--- | :--- |
| **A 天花板** | 并网低 = 做不到 | 收入仍近 **$390M** | 不加仓 |
| **B 时差** | 并网低 = 晚一季入账（假设 B4 ~90 天） | **环比明显高于 Q1**；不要求全年指引上修 | 偏多 |
| **C 打穿全年** | 热到 FY **$3.0–3.4B** 也要上修 | 当季涨 **且** 指引上修 | 更进攻 |

打分 `c·r·s·φ` / `τ·l·e` 只排序，系数是假设，**不进 Sizing**（[`assumption`](pipeline/SCHEMA.md#tbl-assumption)）。

### 4.3 SELECT · 披露前选 B

截止 **2026-08-12 开盘前**。选 **B**：合同电已 >3.5 GW、单已落到 NBIS、$389.7M 那季已写 exceeded expectations → 更像时差，不像做不到。不选 A（把滞后当成天花板）；不选 C（全年指引里已含后加速，打穿是另一跳）。

### 4.4 闭环 · 兑现后对错都写回

| 口径 | B 事先怎么说 | Q2 实际 | 记在谁头上 |
| :--- | :--- | :--- | :--- |
| AI cloud | 环比明显高于 $389.7M | **$575M**（+47.6%） | **B 命中** |
| 集团 | 同上 | **$582.3M**（+46.0%） | **B 命中** |
| ARR | 随产能兑现上移 | $1.92B → **$3.0B** | B 同向 |
| FY 指引 | B **不要求**上修 | 仍 **$3.0–3.4B** | **C 未命中**；B 的边界钉在「只覆盖环比」 |

**选错改哪一层**（先看标签打在谁身上）：

| 情况 | 改哪 | 本例 |
| :--- | :--- | :--- |
| 池里有这条路，选错了 | **SELECT**（`decision_log` 标 wrong，下次降权） | 若选 A：并网低 ≠ 当季做不到；A 仍留在池里 |
| 成真的那条当时没列出来 | **PROPOSE**（补进 `candidate_pool`） | 本例没有 |
| 数抽错了 | **REPRESENT** | 本例没有 |
| B 对了就自动当成 C | 禁止；C 保持单独一跳 | 全年指引没动，C 记错 |

**轨迹先当评测，不先当 SFT。** 一行 = 一次 SELECT：当时看见哪些记录、候选 {A,B,C}、选了谁、各条事先写死的预测、财报后 correct/wrong/mixed。样本是「票 × 季」，先看校准曲线和池子覆盖率；够了再训打分头 P(路径|状态)。抽取轨迹（原文 → 字段）另接 Langfuse，不要和 SELECT 混成一份生成语料。

工程账：[`checks.py`](pipeline/checks.py) 挡住未验证假设进 Sizing；`change_log` 重放当时看见哪几行；`v_health` 看信源是否过期。Langfuse 与打分头均为规划。
