...
1. **PROPOSE（确定点边与候选）**：
   ...
   *   **最小割（哪一环断了最疼）**：持有 NBIS 有 6 条各自独立的理由。少一条（比如某个月光模块数据还没出来），另外 5 条还在，不必立刻平仓。但其中好几条供给理由（光模块在发货、GPU 在买、capex 在花）**都默认同一件事已经发生：机房电已经接上、机器已经在转**。电没接上，这几条理由同一天失效。所以最该盯的不是「有没有 6 条证据」，而是红框那一件：上电有没有发生。
