...
在进行链路推演前，我首先接入并锁定了以下时间点的数据事实：
*   **供给端 (D1)**：海关 HS8517 光模块出口数据，显示 2026 年 5-6 月出货量显著上升（来源：海关总署月报，P1 级）。
*   **需求端 (D3)**：前沿 Lab 融资额，显示 Reflection 等头部实验室在 2025 Q4 至 2026 Q2 的大额融资陆续到位并转化为算力订单（来源：新闻/Form D，P1 级）。
*   **公司口径**：NBIS 管理层在 2026 Q1 及此前电话会中给出的上电量预期与指引。

基于上述确定的数据源，我跑通了以下实战闭环：

1. **PROPOSE（确定点边与候选）**：
   *   **点 (Nodes)**：需求端我锚定了 NBIS 的前瞻 KPI——“机房实际上电量（Deployed Power）”，它直接决定了公司未来的确收产能。在供给端，由于 NBIS 业绩受制于上游硬件供给与下游客户购买力，我选取了 D1 算力维度（光模块出口）与 D3 资本维度（Lab 融资额）作为核心驱动变量。
