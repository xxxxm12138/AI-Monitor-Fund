...
1. **PROPOSE（确定点边与候选）**：
   ...
   *   **为什么叫最小割**：图论用语，意思是「最少拆掉几个点，左边的证据就到不了 NBIS」。借来指标哪一环是公共瓶颈。旭创、新易盛、云厂 capex 三条供给边在库里先汇到「集群在建」（`k_cluster_build`），这个点拆掉，三条一起没，所以这一子图的最小割 = 1。
   *   **上电不是那三条的汇合点**：库里上电是单独一条边 `e_power_nbis`（`m_nbis_capacity` → 产能执行），只连合同电力。上一版把光 / GPU / capex 叠进一个盒子再接到上电，所以看起来红框只连了一条，画错了。没通电变不成收入，是业务判断，不是图上那三条边都经过上电。GPU 走 `k_gpu_supply_cost`，本体 backlog 直连 NBIS。
