...
为了验证系统有效性，我们需要一个最具代表性的深度样板。基于该基金 2026 Q2 的 13F 披露文件，**NBIS（占据 51% 核心仓位）** 不仅权重最大，且在业务逻辑上处于 AI 基建价值链（L2 云与平台）的枢纽位置，是衔接上游算力与下游应用的最佳观测点。我们将理论在此完全实例化，跑通了它的实战闭环：

1. **PROPOSE（确定点边与候选）**：
   *   **点 (Nodes)**：需求端我们首先对 NBIS 的业务模式进行拆解，其核心 KPI 并非后验的营收，而是前瞻的“机房实际上电量（Deployed Power）”，这直接决定了其未来的确收产能。而在供给端，因为 NBIS 是算力租赁方，其业绩受制于上游的硬件供给与下游客户的购买力。因此，我们精准抓取了 D1 算力维度（“光模块排产/海关出口”）与 D3 资本维度（“前沿 Lab 融资额”），它们构成了最强因果驱动。
