...
这一节用持仓最重的 **NBIS**（2026 Q2 13F 约 51%）把前面的点边与计算循环跑通一遍：披露前只使用当时可见的事实，枚举互斥解释并选定一条路径；披露后再把结果回写，看哪条成立、下次该改哪一层。窗口是 Q1 股东信日 **2026-05-13** 到 Q2 披露日 **2026-08-12**。海关序列本轮仍是 placeholder，不参与推断。
...
观测到的信号分两块：上游已经扩张——光模块营收大增、Reflection **$1B** 合同落地、合同电力 **>3.5 GW**；公司侧尚未同步——并网仍 **800 MW–1 GW**，Q1 AI cloud 仍是 **$389.7M**。冲突在「供给和订单已经起来，并网与上季收入还没跟上」。决策空间是三种互斥解释：A 天花板（并网卡死当季）、B 时差（入账滞后）、C 打穿全年（当季加速且 FY 指引上修）。图上是三条并行传导链，并网不接入光模块供给（[`graph_data.py`](pipeline/graph_data.py)）。
