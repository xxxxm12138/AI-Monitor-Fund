...
*   **观测信号池 (Signal Set)**：将需求侧锚定与上表（监测层 × 七维）对齐后，本轮落地的观测信号按层收口如下——**前沿**：D5 评测 / 模型卡 / lab 技术稿，辅以 D2 数据权属类信号；**创业 / 未上市**：D3 lab 融资与合同、D1 中游排产与出货（含中国光模块地面数据）、neocloud 供给、早期 D6 采用、以及先打到中游的 D7 政策冲击；**上市公司**：D1 云厂 capex 与光模块营收、持仓本体 KPI（收入 / ARR / 并网 / 合同电力等）、可比公司披露、可对账的 D6 / D7 事件。其中中国供应链地面序列相对美股 KPI 具有领先期，构成本轮 Nowcast 的主观测通道。
