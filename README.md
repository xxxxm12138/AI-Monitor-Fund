...
**使用对象（假设）**：以研究员和 PM 为主，数据岗为辅。前者在持有期内跟踪投资逻辑是否变化、在财报前判断预期差、并在必要时核验管理层表述；后者主要进入判断记录视图，核验执行方、校准阶段与事后回写。采集与入库仍在库侧完成。
...
这一节用 **NBIS**（2026 Q2 13F 约 51%）把前面的点边与计算循环跑通一遍：披露前只使用当时可见的事实，枚举互斥解释并选定一条路径；披露后再把结果回写，看哪条成立、下次该改哪一层。窗口是 Q1 股东信日 **2026-05-13** 到 Q2 披露日 **2026-08-12**。海关序列本轮仍是 placeholder，不参与推断。
...
选 **B** 不是把时差写成已知事实，而是在三条均未被证伪的假说里取一条**工作假设**。4.1 的事实仍是事实：合同电力 **>3.5 GW**、Reflection **$1B** 已落到 NBIS、Q1 已披露 exceeded expectations。由此推断更符合入账时滞、而非当期硬约束，并调用假设 B4（产能领先收入约 90 天，[`assumption`](pipeline/SCHEMA.md#tbl-assumption)）。排除 A：把时滞误判为约束。排除 C：全年指引已内含后半程加速，上修是独立跳跃，本轮证据够不到。窗口截止 **2026-08-12 开盘前**，选择写入 [`decision_log`](pipeline/SCHEMA.md#tbl-decision_log)，等披露后回写。

落地后这条 SELECT 由**研究员 / PM** 拍板。系统（Code / LLM）只负责摊开候选空间、证伪条件与排序分数；路径选择本身进假设台账，未完成校准前禁止自动执行。
...
> **核心思路**：按假设，面板以研究员和 PM 为主、数据岗为辅；后者主要看判断记录。四块屏幕对应持仓扫描、事实溯源、判断记录和持仓传导，不是四套系统。在线：[AI Monitoring System](https://creatoraix.top/AI-Monitoring-System/)。
