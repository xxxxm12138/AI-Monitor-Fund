...
1. **PROPOSE（确定点边与候选）**：
   *   **点 (Nodes)**：三条平行链，不是「所有证据先经过上电再到达 NBIS」。
       *   **供给**：旭创 / 新易盛 / 云厂 capex → 中间机制「集群在建」`k_cluster_build`（[`graph_data.py` ↗](pipeline/graph_data.py)）。
       *   **需求**：lab 融资 → 「算力采购力」→ 合同（Reflection $1B）。
       *   **公司自己**：合同电力、并网上电、backlog、收入指引。上电要盯，因为它决定在建产能能不能变成当期收入；它不是供给三条边的汇合点。
   *   **边与权重**：供给、需求、本体各有独立边（[`edge_registry` ↗](pipeline/SCHEMA.md#tbl-edge_registry)）。节点质量用五因子（r 可靠度 / e 独家性 / l 领先性 / s 信噪比 / φ 频率），双重打分只用于排序：
       *   **可交易度 (Tradable)** = `c · r · s · φ`。例如，旭创季报（P1，可数值化）排序靠前；海关 HS8517 因 C1 未标定，只参与方向、不进量级。
       *   **预警度 (Warning)** = `τ · l · e`。例如，Lab 融资停滞（独家早期信号，长领先期）提示需求断档风险。
       
       上述系数目前登记为假设值，不进入仓位 Sizing（见 [`assumption` ↗](pipeline/SCHEMA.md#tbl-assumption)）。点状快照不够做 T1 回测；补齐时序后用实测领先期与命中率替换先验。
   *   **最小割只管供给子图**：旭创、新易盛、云厂 capex 三条边共用「集群在建」，这个点拆掉，三条供给同时断，所以这一子图最小割 = 1。需求边、上电边、backlog 不经过这个点，全图不是最小割 = 1。

<div align="center">
<img src="assets/readme/evidence-nbis.svg" alt="NBIS 边：三条供给汇集群在建；上电是单独产能边" width="88%">
</div>
...
3. **PREDICT 与 SELECT（交叉对账与证伪）**：
   ...
   *   **证伪是 SELECT 判断，不是新事实。** 对的是三条平行链，不是「光模块经过上电」：
       *   供给子图（集群在建）：旭创 / 新易盛 Q1 已放量（4 月可知）。
       *   需求子图：融资已落到 Reflection ↔ NBIS **$1B** 合同（7-14 可知）。
       *   公司子图：并网仍给 **800 MW–1 GW**，全年营收锁 **$3.0–3.4B**，Q1 AI cloud 仍是 **$389.7M**（5-13 最后一版指引）。
       供给和需求已经在上，公司口径还停在保守档。比较窗口 **2026-05-13 → 2026-08-12**（91 天）。假设 B4「上电领先收入 ~90 天」只用在公司子图，不是光学领先上电，且仍为 assumption。Q2 兑现的是公司子图往上修：
   ...
   全年营收指引 $3.0–3.4B 在 Q2 被重申、没有被打穿。被证伪的是「公司子图仍按 Q1 平台期外推」。库内无卖方一致预期，预期差只对公司上期实际与公司指引。
