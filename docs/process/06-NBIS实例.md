...
> **为什么这么做** — 范式讲得再漂亮,不落到真数就是 PPT;要证明"链稳不稳、可回溯",必须拿最大持仓真跑一遍。
> **思路** — 用 NBIS(51% 权重)把整套 schema 灌满真数:供给读 ⟷ 需求读两端对账,每字段真实值、每源可得性分级、每条边验证 tier 与"怎么跑",到期用真值给预测打分。
> **目标** — 一个"**真实数据 + 思路过程 + 已兑现结果**"的端到端样板,诚实标出跑不通的地方(placeholder / 付费 / 点少)。数据 as-of 2026-09;来源分五级 provenance。

<div align="center">
<img src="../../assets/readme/nbis.svg" alt="NBIS 供给读 ⟷ 需求读两端对账 + 已兑现" width="100%">
</div>
