...
.guide-head .gt{font-size:14px;font-weight:700}.guide-head .gs{font-size:11px;color:var(--mut);margin-top:3px;line-height:1.45}
...
let tab='overview', ticker='NBIS', mmZ=0, mmS=1, mmX=0, mmY=0, guideOn=false, guideFocus='';
const GUIDE={
 overview:[
...
const LLM={health:{esc:0.14,eval:0.85,halluc:1,cost:'$12/日',todo:5,shadow:3,assisted:1},
 ...
 queue:[{sev:'high',kind:'幻觉旗标',txt:'背离解释初稿 · r158 · 引用了不存在的公告',type:'d_divergence_draft'},
  {sev:'high',kind:'低分待复核',txt:'背离解释初稿 · r166 · eval 0.58 < 阈 0.70',type:'d_divergence_draft'},
  {sev:'high',kind:'低分待复核',txt:'主题聚类 · 顶刊批次 #12 · eval 0.61',type:'d_topic_cluster'},
  {sev:'mid',kind:'OOD 逃逸',txt:'源分类 · 新型来源(播客转写)· 已自动转人工',type:'d_source_classify'},
  {sev:'act',kind:'待晋升审批',txt:'源分类五元组 · shadow 连续 6 窗 ≥ 人 → 升 assisted?',type:'d_source_classify'}],
...
function renderLLM(){const H=LLM.health;
 ...
 let h=`<div class="tag w" style="display:inline-block;margin-bottom:10px">LLM decisions = 0 active · preview layout</div>
  ...
  <div class="kcard" style="margin-bottom:12px">${LLM.queue.map(z=>`<div class="ln" style="padding:8px 12px"><span class="tag ${z.sev==='high'?'w':'n'}">${z.kind}</span><span class="v" style="flex:1">${z.txt}</span><span class="mini" style="color:var(--ac);cursor:pointer" onclick="setJudType('${z.type}')">Trace ›</span></div>`).join('')}</div>
 ...
 if(judType&&LLM.inst[judType]){const t=LLM.types.find(x=>x.id===judType),ins=LLM.inst[judType];
  h+=`<div style="margin-top:12px;border:1px solid var(--ac-bd);border-radius:8px;background:#fbfbff;padding:10px 12px"><div style="font-size:12px;font-weight:700;color:var(--ac);margin-bottom:6px">${t.name} · eval ${t.eval} · escalation ${(t.esc*100).toFixed(0)}% · ${t.prompt} · Langfuse</div>
   <table><thead><tr><th>ID</th><th>Target</th><th>Choice</th><th class="r">Conf</th><th>Eval</th><th>Trace</th></tr></thead><tbody>${ins.map(x=>`<tr><td class="mini">${x.dec}</td><td class="mini">${x.target}</td><td style="font-size:11px">${x.choice}${x.esc?' <span class="tag w">Escalated</span>':''}</td><td class="r num">${x.conf}</td><td class="mini">${x.ev.join(' · ')}</td><td><a href="https://langfuse.example/traces/${x.trace}" target="_blank" style="color:var(--ac)">↗</a></td></tr>`).join('')}</tbody></table></div>`}
 return h}
...
function renderGuide(){
 ...
 el.innerHTML=`<div class="guide-head"><div class="gt">Guide · ${tabLab}</div><div class="gs">Click a module on the page or in this panel. Switch tabs to switch guides. Click dimmed area to clear.</div></div><div class="guide-body">${body}</div><div class="guide-foot">${items.length} modules · two-way spotlight</div>`;
 applyGuideHL();
...
function hdr(){
 ...
 document.getElementById('hdr').innerHTML=`
  ...
  <div class="grow"></div>
  <button class="guide-btn ${guideOn?'on':''}" onclick="toggleGuide()">Guide</button>
...
function render(){hdr();toprow();rail();ctrl();stage();paintMermaid();renderGuide()}
render();
</script></body></html>"""
