...
<a id="tbl-stg_observation"></a>
### `stg_observation`  · 317 行

| 列 | 含义 | 设计出处 | 有值 | 可推导→视图 |
|---|---|---|---|---|
| `record_id` | 登记记录主键 r### | 实现 | 317/317 |  |
| `metric_id` | 指标 id | doc21 | 317/317 |  |
| `entity` | 实体名（文本，路由后映射 entity_id） | 实现 | 317/317 |  |
| `obs_type` | actual 实测 / prior 上期 / guidance 指引 / target 目标 / event 事件 / state 状态 / position 仓位 / computed 计算值 | 实现 | 317/317 |  |
| `value` | 值（文本原样） | 实现 | 317/317 |  |
| `unit` | 单位 / 口径短语 | 实现 | 317/317 |  |
| `period` | 期间 | 实现 | 317/317 |  |
| `knowledge_time` | 何时可知，原文粒度 | doc17 | 317/317 |  |
| `source` | 来源名 + URL | doc23 | 317/317 |  |
| `provenance` | P1–P5 | doc23 | 317/317 |  |
| `note` | 备注 / 原文短引 / 假设 id | 实现 | 317/317 |  |
| `routed_to` | 路由到的资产表 | 实现 | 317/317 |  |

<a id="tbl-edge_registry"></a>
### `edge_registry`  · 36 行

| 列 | 含义 | 设计出处 | 有值 | 可推导→视图 |
|---|---|---|---|---|
| `edge_id` | 边主键 e_*（真源 E） | doc21 §二.5 | 36/36 |  |
| `from_metric` | 上游 AI 节点 | doc21 §二.5 | 36/36 |  |
| `to_ticker` | 持仓票 | doc21 §二.5 | 36/36 |  |
| `side` | supply / demand / regime / compete / self（蝴蝶结左右 / 顶带） | doc25 §五 | 36/36 |  |
| `edge_type` | 六类边 E1 本体 … E6 资金人才流 | doc25 §1.1 | 36/36 |  |
| `hops` | 跳数 = 从票的利润因子反推到节点跨过的因果箭头数 | doc25 §1.2 | 36/36 |  |
| `map_path` | 中间机制节点（→ 分隔）；跳数 = 中间节点数 + 1 | doc25 §1.2 / A10 | 36/36 |  |
| `mechanism` | 机制描述 | doc21 §二.5 | 36/36 |  |
| `cert_tier` | T1 可回测 / T2 结构 + 真值对账 + 敏感性 / T3 方向不下注 | doc25 §六 | 36/36 |  |
| `cert_method` | 验证方法 | doc25 §六 | 36/36 |  |
| `cert_score` | 验证得分（回测 / 对账写回） | doc21 §二.5 | 1/36 |  |
| `lead_measured` | 实测领先期 | doc21 §二.5 | 1/36 |  |
| `tradable_recorded` | 记录的可交易度（doc28 手写 / 早期计算）；推导值见 v_edge_calc | doc25 §1.3 | 35/36 |  |
| `warning_recorded` | 记录的预警度 | doc25 §1.3 | 35/36 |  |
| `is_key` | 重点 ★ 计数 | doc25 §二 | 36/36 |  |
| `evidence_ids` | 证据记录 / trace id（JSON） | doc21 §二.5 | 36/36 |  |
| `evidence` | 证据描述 | 实现 | 35/36 |  |
| `owner` | 分工 | doc09 | 36/36 |  |
| `status` | testing / production / deprecated | doc21 §二.5 | 36/36 |  |
| `notes` | 备注（含因子赋值理由） | 实现 | 36/36 |  |
| `last_validated` | 最近验证日 | doc21 §二.5 | 35/36 |  |
| `tradable_score` | 可交易度 = c·r·s·φ | doc21 §6.3 | — | → v_edge_calc |
| `warning_score` | 预警度 = τ·l·e | doc21 §6.3 | — | → v_edge_calc |

<a id="tbl-entity_master"></a>
### `entity_master`  · 50 行
...
<a id="tbl-metric_entity"></a>
### `metric_entity`  · 105 行

| 列 | 含义 | 设计出处 | 有值 | 可推导→视图 |
|---|---|---|---|---|
| `metric_id` | 指标 | 实现 | 105/105 |  |
| `entity_id` | 实体（一指标可挂多实体，如 hyperscaler 合计） | 实现 | 105/105 |  |

<a id="tbl-metric_registry"></a>
### `metric_registry`  · 85 行

| 列 | 含义 | 设计出处 | 有值 | 可推导→视图 |
|---|---|---|---|---|
| `metric_id` | 指标主键 m_*（真源 R 的节点） | doc21 §二 | 85/85 |  |
| `name` | 名称 | doc21 §二 | 85/85 |  |
| `entity_id` | 主实体 | doc21 §二 | 85/85 |  |
| `ticker_or_entity` | 登记时的实体 / 票标签（原文） | 实现 | 85/85 |  |
| `entry` | 入口：持仓（需求侧）/ AI（供给侧） | doc33 §〇 | 85/85 |  |
| `track` | 赛道 | doc21 §二 | 68/85 |  |
| `layer` | 价值栈 L1–L6（由主实体继承） | doc21 §二 | 72/85 |  |
| `dim` | AI 发展七维 D1–D7 | doc24v2 | 85/85 |  |
| `data_class` | 1 量价 / 2 事件 / 3 观点 / 4 前沿（决定落哪张 fct） | doc17 §一 | 85/85 |  |
| `signal_role` | predictor / expectation_base / regime / arbiter / human_input | doc17 §2.1 | 85/85 |  |
| `source_ids` | 来源 id 列表（JSON） | doc21 §二 | 68/85 |  |
| `source_family` | 来源族描述 | 实现 | 85/85 |  |
| `frequency` | 更新频率 | doc21 §二 | 85/85 |  |
| `lead_time_est` | 领先期先验（补丁④，假设 B*） | doc18 补丁④ / doc30 | 83/85 |  |
| `kou_jing` | 口径定义 | doc21 §二 | 68/85 |  |
| `unit` | 单位 | doc21 §二 | 68/85 |  |
| `owner` | 分工 | doc09 | 85/85 |  |
| `upstream_deps` | 上游依赖（JSON） | doc21 §二 | 68/85 |  |
| `availability` | 🟢一手公开 🟡待钉一手 🔵付费 ⚪placeholder / 合规 | doc28 §一 | 85/85 |  |
| `next_release` | 下次发布 | 实现 | 83/85 |  |
| `next_release_basis` | 依据：公司公告 / 规则推定 / 估计 / 待定 | 实现 | 83/85 |  |
| `status` | testing / production / deprecated | doc21 §二 | 85/85 |  |
| `fill_status` | 填数状态：已填 / 待填 / 待钉一手 / 付费定性 / placeholder / 合规受限 | 实现 | 85/85 |  |
| `notes_assumption_ids` | 备注与引用的假设 id | doc30 | 83/85 |  |
| `last_validated` | 最近验证日 | doc21 §二 | 75/85 |  |
...
<a id="tbl-fct_event"></a>
### `fct_event`  · 92 行

| 列 | 含义 | 设计出处 | 有值 | 可推导→视图 |
|---|---|---|---|---|
| `record_id` | 记录主键，来自接入层 stg_observation | doc17 §2.1 溯源 | 92/92 |  |
| `metric_id` | 挂到哪个指标（真源 R） | doc17 §2.1 关联 | 92/92 |  |
| `entity_id` | 主实体（entity_master） | doc17 §2.1 关联 | 92/92 |  |
| `knowledge_time` | 何时可知（as-of），按原文粒度：日 / 月 / 季 / 年；回测生命线 | doc17 §2.1 溯源 | 92/92 |  |
| `knowledge_date` | knowledge_time 解析成日期（月→15 日；年 / 季→NULL），供排序与窗口，不伪造精度 | 实现 | 81/92 |  |
| `source_id` | 来源（source_master） | doc17 §2.1 溯源 / doc23 | 92/92 |  |
| `source_url` | 来源 URL | doc17 §2.1 溯源 | 68/92 |  |
| `anchor` | 原文锚点 / 页码 / 时间码 / 短引 | doc17 §2.1 溯源 | 62/92 |  |
| `snapshot_id` | 原始快照 id（raw 湖） | doc17 §2.1 溯源 | 60/92 |  |
| `ingest_time` | 入库时间 | doc17 §2.1 溯源 | 92/92 |  |
| `provenance` | 可信度五级 P1 一手 / P2 电话会口径 / P3 二手 / P4 估算预测 / P5 设计层评分 | doc23 §一 | 92/92 |  |
| `owner` | 人机分工 A AI独立·人抽检 / B AI初稿·人必复核 / C 人定义·AI执行 / D 人定 | doc09 / doc17 §2.1 质量 | 92/92 |  |
| `confidence` | 抽取置信度 | doc17 §2.1 质量 | 54/92 |  |
| `code_score` | 代码闸门得分（JSON；Langfuse trace 锚点） | doc17 §2.1 质量 | — |  |
| `judge_score` | LLM judge 得分 | doc17 §2.1 质量 | — |  |
| `status` | pass / pending（待人工）/ rejected | doc17 §2.1 质量 | 92/92 |  |
| `superseded_by` | 旧值不删，标被哪条记录取代（与 revision_flag 同构） | doc23 §五 | 14/92 |  |
| `note` | 备注 / 口径说明 | 实现 | 92/92 |  |
| `event_type` | funding / launch / personnel / order / approval / contract / policy / litigation / investment / pricing / other | doc21 §三（扩展） | 92/92 |  |
| `subject_entity` | 主体 | doc21 §三 | 92/92 |  |
| `object_entity` | 客体 | doc21 §三 | 44/92 |  |
| `relation` | 关系 | doc21 §三 | 46/92 |  |
| `amount` | 金额 | doc21 §三 | 21/92 |  |
| `amount_text` | 无法数值化的金额原文 | 实现 | 92/92 |  |
| `currency` | 币种 | doc21 §三 | 36/92 |  |
| `amount_type` | one_time / multi_year_cap / annualized / range（补丁②） | doc18 补丁② | 42/92 |  |
| `is_estimate` | 是否估算 | doc18 补丁② | 92/92 |  |
| `round` | 融资轮次 | doc21 §三 | 9/92 |  |
| `event_date` | 事件日期 | doc21 §三 | 81/92 |  |
| `event_date_text` | 事件日期原文 | 实现 | 92/92 |  |

<a id="tbl-fct_frontier"></a>
### `fct_frontier`  · 23 行

| 列 | 含义 | 设计出处 | 有值 | 可推导→视图 |
|---|---|---|---|---|
| `record_id` | 记录主键，来自接入层 stg_observation | doc17 §2.1 溯源 | 23/23 |  |
| `metric_id` | 挂到哪个指标（真源 R） | doc17 §2.1 关联 | 23/23 |  |
| `entity_id` | 主实体（entity_master） | doc17 §2.1 关联 | 23/23 |  |
| `knowledge_time` | 何时可知（as-of），按原文粒度：日 / 月 / 季 / 年；回测生命线 | doc17 §2.1 溯源 | 23/23 |  |
| `knowledge_date` | knowledge_time 解析成日期（月→15 日；年 / 季→NULL），供排序与窗口，不伪造精度 | 实现 | 20/23 |  |
| `source_id` | 来源（source_master） | doc17 §2.1 溯源 / doc23 | 23/23 |  |
| `source_url` | 来源 URL | doc17 §2.1 溯源 | 16/23 |  |
| `anchor` | 原文锚点 / 页码 / 时间码 / 短引 | doc17 §2.1 溯源 | 17/23 |  |
| `snapshot_id` | 原始快照 id（raw 湖） | doc17 §2.1 溯源 | 14/23 |  |
| `ingest_time` | 入库时间 | doc17 §2.1 溯源 | 23/23 |  |
| `provenance` | 可信度五级 P1 一手 / P2 电话会口径 / P3 二手 / P4 估算预测 / P5 设计层评分 | doc23 §一 | 23/23 |  |
| `owner` | 人机分工 A AI独立·人抽检 / B AI初稿·人必复核 / C 人定义·AI执行 / D 人定 | doc09 / doc17 §2.1 质量 | 23/23 |  |
| `confidence` | 抽取置信度 | doc17 §2.1 质量 | — |  |
| `code_score` | 代码闸门得分（JSON；Langfuse trace 锚点） | doc17 §2.1 质量 | — |  |
| `judge_score` | LLM judge 得分 | doc17 §2.1 质量 | — |  |
| `status` | pass / pending（待人工）/ rejected | doc17 §2.1 质量 | 23/23 |  |
| `superseded_by` | 旧值不删，标被哪条记录取代（与 revision_flag 同构） | doc23 §五 | 3/23 |  |
| `note` | 备注 / 口径说明 | 实现 | 23/23 |  |
| `doc_id` | 文献 / 榜单 id | doc21 §三 | 3/23 |  |
| `topic_cluster` | 主题簇 | doc21 §三 | 4/23 |  |
| `method` | 方法 | doc21 §三 | 4/23 |  |
| `benchmark` | 基准 | doc21 §三 | 17/23 |  |
| `score` | 分数 | doc21 §三 | 2/23 |  |
| `score_text` | 无法数值化的分数原文 | 实现 | 21/23 |  |
| `institution` | 机构 | doc21 §三 | 23/23 |  |
| `authors` | 作者（JSON） | doc21 §三 | 1/23 |  |
| `citation_velocity` | 引用速度 | doc21 §三 | 0/23 |  |
| `source_stance` | neutral / vendor_pr / social（证伪闸） | doc29 §四 | 21/23 |  |
| `verifiability` | reproducible / third_party_verified / claimed_only / rumor（证伪闸） | doc29 §四 | 21/23 |  |
| `industry_impact` | 产业影响 高 / 中 / 低（影响力闸，owner D；量化占位 C8） | doc29 §四 | 20/23 |  |
| `impact_rationale` | 影响力理由 | doc29 §四 | 20/23 |  |

<a id="tbl-fct_opinion"></a>
### `fct_opinion`  · 8 行
...
<a id="tbl-source_master"></a>
### `source_master`  · 176 行

| 列 | 含义 | 设计出处 | 有值 | 可推导→视图 |
|---|---|---|---|---|
| `source_id` | 来源主键 src_* | doc23 §二 | 176/176 |  |
| `name` | 名称 | doc23 §二 | 176/176 |  |
| `publisher` | 发布方 | doc23 §二 | 100/176 |  |
| `source_tier` | P1–P5 | doc23 §二 | 174/176 |  |
| `source_kind` | ir_release / exchange_filing / prospectus / customs / call_transcript / broker_research / news_media / data_aggregator / official / court / company_blog / api | doc23 §二（扩展） | 132/176 |  |
| `data_class` | 供给哪几类数据（JSON） | doc23 §二 | 176/176 |  |
| `covers_entity` | 覆盖实体（JSON） | doc23 §二 | 176/176 |  |
| `covers_metric` | 覆盖指标（JSON） | doc23 §二 | 176/176 |  |
| `url` | URL | doc23 §二 | 127/176 |  |
| `access` | open / paywall / blocked | doc23 §二 | 176/176 |  |
| `frequency` | 频率 | doc23 §二 | 99/176 |  |
| `reliability_note` | 为何是这个 tier / 软肋 | doc23 §二 | 117/176 |  |
| `verify_status` | confirmed / pending / stale / placeholder | doc23 §二 | 176/176 |  |
| `superseded_by` | 旧源不删，标被谁取代 | doc23 §二 | 10/176 |  |
| `first_seen` | 首见 | doc23 §二 | 158/176 |  |
| `last_validated` | 最近核验 | doc23 §二 | 176/176 |  |

<a id="tbl-assumption"></a>
### `assumption`  · 42 行
...
<a id="tbl-metric_factor"></a>
### `metric_factor`  · 84 行

| 列 | 含义 | 设计出处 | 有值 | 可推导→视图 |
|---|---|---|---|---|
| `metric_id` | 节点 | doc21 §6.1 | 84/84 |  |
| `r` | 可靠性（provenance→r，A8） | doc21 §6.1 | 84/84 |  |
| `e` | 独家性 | doc21 §6.1 | 84/84 |  |
| `l` | 固有领先性 | doc21 §6.1 | 84/84 |  |
| `s` | 可观测 / 信噪 | doc21 §6.1 | 84/84 |  |
| `phi` | 频率适配 | doc21 §6.1 | 84/84 |  |
| `rationale` | 赋值理由 | 实现 | 84/84 |  |
| `owner` | 分工 | doc09 | 84/84 |  |
| `status` | draft / reviewed / retired | 实现 | 84/84 |  |

<a id="tbl-observation_direction"></a>
### `observation_direction`  · 317 行

| 列 | 含义 | 设计出处 | 有值 | 可推导→视图 |
|---|---|---|---|---|
| `record_id` | 记录 | A11 | 317/317 |  |
| `direction` | ↑增强 / ↓削弱 / →中性 / !冲击需人判 / —不适用，相对所喂持仓票的 thesis | A11 | 317/317 |  |
| `direction_note` | 一句理由 | A11 | 317/317 |  |
| `owner` | 分工 | doc09 | 317/317 |  |
| `status` | draft / reviewed / retired | 实现 | 317/317 |  |

<a id="tbl-source_override"></a>
...
<a id="tbl-change_log"></a>
### `change_log`  · 3226 行

| 列 | 含义 | 设计出处 | 有值 | 可推导→视图 |
|---|---|---|---|---|
| `change_id` | 改动 id = migration_id-序号 | 元数据 | 3226/3226 |  |
| `migration_id` | 属于哪次改库 | 元数据 | 3226/3226 |  |
| `table_name` | 改了哪张表 | 元数据 | 3226/3226 |  |
| `pk` | 哪一行（主键值） | 元数据 | 3226/3226 |  |
| `column_name` | 哪一列；整行新增记 (insert) | 元数据 | 3226/3226 |  |
| `old_value` | 改前值（文本） | 元数据 | 715/3226 |  |
| `new_value` | 改后值（文本） | 元数据 | 3218/3226 |  |
| `basis` | 依据：原文页码 / 快照 id / doc 编号 / 规则 | doc14 A1 可溯源 | 3226/3226 |  |
| `owner` | A–D | doc09 | 3226/3226 |  |
| `applied_at` | 应用时间 | 元数据 | 3226/3226 |  |

<a id="tbl-decision_fork"></a>
### `decision_fork`  · 39 行
...
<a id="tbl-decision_log"></a>
### `decision_log`  · 95 行

| 列 | 含义 | 设计出处 | 有值 | 可推导→视图 |
|---|---|---|---|---|
| `dec_id` | 判断实例 id = migration_id-dec 序号 | PLAN-decision-layer §二 | 95/95 |  |
| `decision_id` | 哪类判断 | PLAN-decision-layer §二 | 95/95 |  |
| `target_table` | 判断对象所在表 | PLAN-decision-layer §二 | 95/95 |  |
| `target_pk` | 对象主键 | PLAN-decision-layer §二 | 95/95 |  |
| `target_column` | 列级判断的列 | PLAN-decision-layer §二 | 72/95 |  |
| `state_anchor` | 判断时刻输入的引用 JSON（record_id 集合 + as_of），判断可复现 | JEV-intelligence-graph §四.2 | 95/95 |  |
| `candidates_shown` | 判断时刻 registry.candidates 快照 | PLAN-decision-layer §二 | 95/95 |  |
| `choice` | 选择 | PLAN-decision-layer §二 | 95/95 |  |
| `confidence` | 置信度 0–1；人按 H/M/L 写映射 .9/.6/.3（C21） | PLAN-decision-layer §六 Q1 | 46/95 |  |
| `executor_kind` | rule / human / llm / model | PLAN-decision-layer §二 | 95/95 |  |
| `executor` | 本次判断的执行者 | doc09 | 95/95 |  |
| `escalated` | 是否从默认执行者升级（R16 分子） | PLAN-decision-layer §二 | 95/95 |  |
| `basis` | 依据（六元组之一） | doc14 A1 可溯源 | 95/95 |  |
| `decided_at` | 判断时间 | 实现 | 95/95 |  |
| `retro` | 回填行：confidence 允许空，禁止编造 | PLAN-decision-layer P3 | 95/95 |  |
| `outcome` | 事后对错 correct / wrong / mixed / unresolved | JEV-intelligence-graph §四.2 | 3/95 |  |
| `outcome_basis` | 回填依据 | PLAN-decision-layer §六 Q4 | 3/95 |  |
| `outcome_at` | 回填日期 | 实现 | 3/95 |  |
| `note` | 说明 | 实现 | 92/95 |  |

<a id="tbl-decision_registry"></a>
### `decision_registry`  · 12 行

| 列 | 含义 | 设计出处 | 有值 | 可推导→视图 |
|---|---|---|---|---|
| `decision_id` | 判断类型主键 d_* | PLAN-decision-layer §二 | 12/12 |  |
| `name` | 名称 | PLAN-decision-layer §二 | 12/12 |  |
| `question` | 这类判断在问什么（一句） | PLAN-decision-layer §二 | 12/12 |  |
| `state_schema` | 输入 state 由哪些表列 / 视图构成（= 未来专用模型的输入契约） | JEV-intelligence-graph §四.1 | 12/12 |  |
| `candidates` | 候选词表 JSON 数组（= 输出契约）；开放候选 = open→candidate_pool | JEV-intelligence-graph §四.1 | 12/12 |  |
| `output_home` | 判断结果写回哪张表哪列（表.列；复合列 + 连接；只落 decision_log 则空） | PLAN-decision-layer §二 | 8/12 |  |
| `executor_kind` | rule / human / llm / model | PLAN-decision-layer §二 | 12/12 |  |
| `executor` | A–D / 规则名 / 模型名 | doc09 | 12/12 |  |
| `freq_est` | 发生频率量级 | PLAN-decision-layer §二 | 12/12 |  |
| `escalation` | 升级规则：首版 = 现状描述，数值阈值挂 C19 | PLAN-decision-layer §六 Q3 | 12/12 |  |
| `calib_status` | human → shadow → assisted → auto（doc30 状态机平移） | JEV-intelligence-graph §四.2 | 12/12 |  |
| `assumption_ids` | 挂的假设 | doc30 | 7/12 |  |
| `owner` | A–D | doc09 | 12/12 |  |
| `status` | draft / reviewed / retired | 实现 | 12/12 |  |
| `design_ref` | 设计出处 | 元数据 | 12/12 |  |
| `notes` | 说明 | 实现 | 12/12 |  |
| `research_tier` | 投研重要性分层：A 直接投研判断 / B 准入·可信 / C 运维·流程（决定复核与注意力分配的优先级） | E50 | 11/12 |  |
...
<a id="tbl-migration_log"></a>
### `migration_log`  · 27 行

| 列 | 含义 | 设计出处 | 有值 | 可推导→视图 |
|---|---|---|---|---|
| `migration_id` | 改库脚本编号 NNNN_name（migrations/） | 元数据 | 27/27 |  |
| `applied_at` | 应用时间 | 元数据 | 27/27 |  |
| `checksum` | 脚本 sha256 前 16 位；应用后脚本不可再改 | 元数据 | 27/27 |  |
| `owner` | 本次改库的 owner | 元数据 | 27/27 |  |
| `n_changes` | 写入 change_log 的行数 | 元数据 | 27/27 |  |
| `note` | 脚本 docstring 第一行 | 元数据 | 27/27 |  |

<a id="tbl-relation_doc"></a>
### `relation_doc`  · 28 行
